...
function countValidPasswords(input) {
    ...
    for (const line of inputLines) {
        const data = line.match(/(?<minCharacterCount>[\d]+) - (?<maxCharacterCount>[\d]+) (?<character>[a-z]): (?<password>[a-z]*)/);
        const {minCharacterCount, maxCharacterCount, character, password} = data.groups;
        const count = (password.match(new RegExp(character, 'g')) || []).length;
        
        if (count >= minCharacterCount && count <= maxCharacterCount) {
            validPasswordCount++;
        }
    }
