const input = `9 - 10 b: bbktbbbxhfbpb
2 - 10 x: xxnxxxwxxsx
5 - 7 w: ghwwdrr
4 - 6 z: nzzjzk
7 - 8 s: szsssswfs
...
function countValidPasswords(input) {
    ...
    for (const line of inputLines) {
        const data = line.match(/(?<characterPositionOne>[\d]+) - (?<characterPositionTwo>[\d]+) (?<character>[a-z]): (?<password>[a-z]*)/);
        const { characterPositionOne, characterPositionTwo, character, password } = data.groups;
        const characterOneMatches = password[characterPositionOne - 1] == character;
        const characterTwoMatches = password[characterPositionTwo - 1] == character;

        if (characterOneMatches + characterTwoMatches === 1) {
            validPasswordCount++;
        }
    }
